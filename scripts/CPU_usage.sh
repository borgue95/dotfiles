...
if [ $(lscpu | grep Architecture: | awk '{print $2}') != "x86_64" ];
then
    freq=$(echo "scale=1; $(cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq) / 1000.0" | bc);
else
...
cpu_usage=$(echo "scale=1; $cpu_usage / 1.0" | bc | awk '{printf("%.0f", $0)}')
#cpu_usage=$(printf "%8s" $cpu_usage)
#printf "%8s\n" $cpu_usage
#echo $cpu_usage

if [ "$(echo "scale=2;$cpu_usage > 75.0 && $cpu_usage < 95.0" | bc -l)" -eq 1 ]; then # 75-95
    printf "<span foreground=\"#FFFC00\"><b>%3s%%</b></span>" $cpu_usage
    printf " @ %s, %s\n" $freq $temp1
elif [ "$(echo "scale=2;$cpu_usage >= 95.0" | bc -l)" -eq 1 ]; then # > 95
    printf "<span background=\"#FF0000\" foreground=\"#FFFFFF\"><b>%3s%%</b></span>" $cpu_usage
    printf " @ %s, %s\n" $freq $temp1
else # < 75
    printf "%3s%% @ %s, %s\n" $cpu_usage $freq $temp1
fi
